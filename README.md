...
A full breakdown of every layer of VISTA — the OCR/ingestion pipeline, the embedding model and why it was chosen, cosine similarity vs. Euclidean distance, how Qdrant's HNSW indexing makes sub-2-second search possible, the clustering methodology, the PII/metadata separation model, evaluation methodology, and known limitations — lives in **[`DOCUMENTATION.md`](./DOCUMENTATION.md)**.
...
- **Evaluation methodology** — concrete metrics (cluster purity vs. ground-truth specialty labels, silhouette score, precision@k) to actually measure whether the system works
